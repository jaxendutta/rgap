...
class ProcessingPipeline:
    ...
    def _process_amendments(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # Vectorized equivalent of: for each group, sort by amendment_number
            # descending, keep the top row as canonical, and fold the rest into
            # amendments_history. A plain Python for-loop with a nested
            # .iterrows() over every group (up to ~hundreds of thousands here)
            # is extremely slow in pandas; this does the same grouping with
            # cython-backed groupby operations instead.

            # Stable sort so ties on amendment_number preserve original row
            # order, same tie-breaking behaviour as the row-by-row version.
            df_sorted = df.sort_values(
                ['_unique_id', 'amendment_number'], ascending=[True, False], kind='mergesort'
            )
            rank = df_sorted.groupby('_unique_id', sort=False).cumcount()

            canonical = df_sorted[rank == 0].copy()
            older = df_sorted[rank > 0]

            if not older.empty:
                # Build one dict per older (superseded) row, dropping null
                # values -- matches the `if pd.notna(row[col])` filter above.
                raw_records = older[history_columns].to_dict('records')
                cleaned_records = [
                    {k: v for k, v in rec.items() if pd.notna(v)} for rec in raw_records
                ]
                history_series = pd.Series(cleaned_records, index=older.index)
                # groupby().agg(list) preserves each group's row order, which
                # is still descending-by-amendment_number from the sort above.
                history_by_id = history_series.groupby(older['_unique_id'], sort=False).agg(list)
                mapped = canonical['_unique_id'].map(history_by_id)
                canonical['amendments_history'] = mapped.apply(
                    lambda v: json.dumps(v) if isinstance(v, list) and len(v) > 0 else None
                )
            else:
                canonical['amendments_history'] = None

            result_df = canonical.drop(columns=['_unique_id']).reset_index(drop=True)
